...
Public Class NewExpenses
    ...
    Public Sub New(ByRef f As ExpensesForm, id As Integer, login As String)
        Me.ef = f
        Me.id = id
        Me.login = login
        InitializeComponent()
    ...
    Private Sub NewExpenses_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        'uzupelnienie comoboxa kategoriami
        connection.Open()
        Using cmd As SqlCommand = New SqlCommand("select * from category where type = 1", connection)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If Regex.IsMatch(TextBoxAmount.Text, "^[0-9 ]+$") Then
            ...
            Dim D As String
            D = MsgBox("Czy jestes pewny że chcesz dodać wydatek o podanych parametrach do bazy?", vbYesNo + vbQuestion, "Pytanie")
            If D = vbYes Then
                'Insert
                connection.Open()
                query = "INSERT INTO expenses (date,amount,category_id,users_id) VALUES ('" & date_val & "','" & amount_val & "','" & category_val & "','" & user_val & "')"
                cmd = New SqlCommand(query, connection)
                cmd.ExecuteNonQuery()
                Me.Close()
                MessageBox.Show("Wydatek dodany do bazy. Będzie on widoczny po wykonaniu aktualizacji!", "Uwaga", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                connection.Close()
        ...
        'If Application.OpenForms().OfType(Of ExpensesForm).Any Then
